...
/**
 * Class <code>WAPPushSender</code> sends a single flash SMS by manipulating the
 * data coding
 * 
...
public class WAPPushSender {
	...
	byte dataCoding = (byte) 0xF5; // Silent SMS 192 // Flash SMS 240 // WAP Push 245 // Default 0
	 ...
	 */
	public WAPPushSender() throws IOException {
	...
	public static void main(String args[]) {
		...
		if ((dest == null)) {
			System.out
					.println("Usage: SMPPSender -dest <dest number on international format> , within quotes if contains whitespaces> [-sender <sender id> [-senderTon <sender ton>] [-senderNpi <sender npi>]]");
			System.exit(0);
		}
		...
		WAPPushSender smppSender = null;
		try {
			smppSender = new WAPPushSender();
		} catch (IOException e) {
			System.out.println("Exception initialising SMPPSender " + e);
		}

		System.out.println("Sending WAP Push message to:" + " to " + dest);
		if (smppSender != null) {
			smppSender.bind();
	...
	private void submit(String destAddress, String sender,
		...
		try {
			...
			byteBuffer.appendByte((byte) 0x06);
			byteBuffer.appendByte((byte) 0x05); // IE Identifier
			byteBuffer.appendByte((byte) 0x04); // IE Data Length
			byteBuffer.appendShort((short) 0x0B84); // Destination Port
			byteBuffer.appendShort((short) 0x23F0); // Originator Port

			byteBuffer.appendString(HexUtil.convertStringToHexString("DC0601AE02056A0045C60C037777772E736d70707365727665722E6f7267000103736d7070736572766572000101"));
