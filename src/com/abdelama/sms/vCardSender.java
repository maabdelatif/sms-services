...
/**
 * Class <code>vCardSender</code> sends a single flash SMS by manipulating the
 * data coding
 * 
 * @author Logica Mobile Networks SMPP Open Source Team, since adapted slightly
 *         by Mohamed Abdelatif
 */
public class vCardSender {
	...
	String validityPeriod = "000000001000000R"; // 10 minutes from now
												// (relative)
	String shortMessage = "";
	int numberOfDestination = 1;
	static String messageId = "";
	byte esmClass = Data.SM_UDH_GSM; // WAP push 64 // Default 0
	byte protocolId = 0;
	byte priorityFlag = 0;
	byte registeredDelivery = 1; // We want a delivery report
	byte replaceIfPresentFlag = 0;
	byte dataCoding = (byte) 0xF5; // Silent SMS 192 // Flash SMS 240 // WAP Push
									// 245 // Default 0
	 ...
	 */
	public vCardSender() throws IOException {
	...
	public static void processEnquireLinkPDU(PDU pdu) {
		...
		System.out.println(getCurrentTime() + " Sending enquire_link_resp "
	...
	public static void processDeliverSMPDU(PDU pdu) {
		...
		System.out.println(getCurrentTime() + " Examining recieved deliverSM "
				+ deliverSM.debugString());

		/**
		 * Format of the delivery report: id:0669703056 sub:001 dlvrd:001 submit
		 * date:1405061655 done date:1405061705 stat:EXPIRED err:254
		 * text:foshfoshfosh@@@@@@@@
		 */

		// Check the message format
		String message = deliverSM.getShortMessage();
		if (message.contains("id:") && message.contains("sub:")
				&& message.contains("dlvrd:") && message.contains("date:")
				&& message.contains("stat:") && message.contains("err")
				&& message.contains("text:")) {
			String[] splited = message.split("\\s+");
			String id = splited[0].substring(splited[0].indexOf(":") + 1);
			String convertedMessageId = Long.toString(Long.parseLong(messageId,
					16));
			/**
			 * Convert both message IDs to Long because there could be leading
			 * zeroes. If this if statement is not executed is is probably
			 * because we have not received a submit_sm_resp
			 */
			if (convertedMessageId != "") {
				if (Long.parseLong(convertedMessageId) == Long.parseLong(id)) {
					String stat = splited[7]
							.substring(splited[7].indexOf(":") + 1);
					String err = splited[8]
							.substring(splited[8].indexOf(":") + 1);
					System.out
							.println("Received deliver report for the message: "
									+ stat + ", with error code: " + err);
				}
			}
		}
		...
		Response response = ((Request) pdu).getResponse();
		System.out.println(getCurrentTime() + " Sending deliver_sm_resp "
				+ response.debugString());
		if (session != null) {
			try {
				session.respond(response);
	...
	public static void processQuerySMRespPDU(PDU pdu) {
		...
		System.out.println(getCurrentTime()
				+ " Received the message status as " + messageState + " "
				+ queryResp.getMessageState() + " " + queryResp.getErrorCode());
	}
	...
	public static void processSubmitSMRespPDU(PDU pdu) {
		...
		System.out.println(getCurrentTime()
				+ " Received Submit SM Resp with message ID as " + messageId);
	}
	...
	public static void main(String args[]) {
		...
		if ((dest == null)) {
			System.out
					.println("Usage: SMPPSender -dest <dest number on international format> , within qoutes if contains whitespaces> [-sender <sender id> [-senderTon <sender ton>] [-senderNpi <sender npi>]]");
			System.exit(0);
		}
		...
		vCardSender smppSender = null;
		try {
			smppSender = new vCardSender();
		} catch (IOException e) {
			System.out.println("Exception initialising SMPPSender " + e);
		}

		System.out.println("Sending vCard to:" + " to " + dest);
		if (smppSender != null) {
			smppSender.bind();
			...
			if (smppSender.bound) {
				...
				while (it.hasNext()) {
					String d = it.next();
					smppSender.submit(d, sender, senderTon, senderNpi);
				}
				...
				// Wait for the validity period to expire, 11 minutes just to be
				// safe
				try {
				...
				// If a delivery receipt is not received after this then send
				// query_sm
				...
				if (messageId != "") {
					try {
						...
						String convertedMessageId = Long.toString(Long
								.parseLong(messageId, 16));
				...
				try {
					System.out.println(getCurrentTime()
							+ " Sending query SM request "
							+ querySM.debugString());
					session.query(querySM);
				} catch (TimeoutException | PDUException
						| WrongSessionStateException | IOException e) {
					// TODO Auto-generated catch block
					e.printStackTrace();
				}
	...
	private void bind() {
		try {
			...
			if (response.getCommandStatus() == Data.ESME_ROK) {
				...
				System.out.println("Bind failed, code "
	 ...
	 * @see SubmitSMResp
	 */
	private void submit(String destAddress, String sender,
		...
		try {
			...
			byteBuffer.appendByte((byte) 6);
			byteBuffer.appendByte((byte) 5); // IE Identifier
			byteBuffer.appendByte((byte) 4); // IE Data Length
			byteBuffer.appendShort((short) 0x23F4); // Destination Port
			byteBuffer.appendShort((short) 0x0000); // Originator Port

			byteBuffer.appendString("BEGIN:VCARD\r\nVERSION:2.1\r\nN:Surname;Name\r\nTEL;PREF:+27812341111\r\nEND:VCARD\r\n");
			request.setMessagePayload(byteBuffer);
			
			if (asynchronous) {
				System.out.println(getCurrentTime() + " Submit request "
						+ request.debugString());
				session.submit(request);
			} else {
				response = session.submit(request);
				messageId = response.getMessageId();
				System.out.println(getCurrentTime() + " Submit response "
						+ response.debugString());
			}
	...
	private void enquireLink() {
		try {
			...
			System.out.println(getCurrentTime() + " Enquire Link request "
					+ request.debugString());
			if (asynchronous) {
				session.enquireLink(request);
			} else {
				response = session.enquireLink(request);
				System.out.println(getCurrentTime() + " Enquire Link response "
						+ response.debugString());
			}
		} catch (Exception e) {
	...
	private void receive() {
		try {
			...
			if (pdu != null) {
				System.out.println(getCurrentTime() + " Received PDU "
						+ pdu.debugString());
				if (pdu.isRequest()) {
					Response response = ((Request) pdu).getResponse();
	...
	private class ReceivedPDUsProcessor implements Runnable {
		...
		public ReceivedPDUsProcessor() {
			System.out
					.println("Starting processor to process received PDUs...");
		}

		public void run() {
			while (true) {
				if (pduListener != null) {
					serverPDUEvent = pduListener
							.getRequestEvent(receiveTimeout);
					if (serverPDUEvent != null) {
						pdu = serverPDUEvent.getPDU();
						commandId = pdu.getCommandId();
						System.out.println(getCurrentTime()
								+ " Processed PDU from Queue: " + commandId);
						switch (commandId) {
						...
						default:
							System.out.println(getCurrentTime()
									+ " Unhandled command ID recieved "
									+ commandId + " "
									+ SUBMIT_SM_RESP_COMMAND_ID);
							break;
						}
					}
				}
